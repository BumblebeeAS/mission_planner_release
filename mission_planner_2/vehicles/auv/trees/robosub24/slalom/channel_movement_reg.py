...
from mission_planner_2.common.util.pose_utils import (
    ...
    create_slalom_clustering_start_srv_request,
    create_slalom_clustering_stop_srv_request,
    create_stamped_pose,
)
...
CLUSTERING_START_SERVICE_NAME = "/auv4/cluster_slalom_tfs/start"
CLUSTERING_STOP_SERVICE_NAME = "/auv4/cluster_slalom_tfs/stop"
...
def create_move_between_layers_root(
    ...
    cluster_srv_start_yaw = py_trees_ros.service_clients.FromConstant(
        name=f"Recluster Transforms from Layer {current_layer} to Layer {next_layer} start (Yaw)",
        service_type=ClusterSlalomTfsStart,
        service_name=CLUSTERING_START_SERVICE_NAME,
        service_request=create_slalom_clustering_start_srv_request(
            input_child_frame_ids=(
                CLUSTERING_IN_CHILDREN_NEAR[:2]
                if current_layer == "zero"
                else CLUSTERING_IN_CHILDREN_NEAR[:1]
            ),
            reset_cache=True,
        ),
    )

    cluster_srv_stop_yaw = py_trees_ros.service_clients.FromConstant(
        name=f"Recluster Transforms from Layer {current_layer} to Layer {next_layer} stop (Yaw)",
        service_name=CLUSTERING_STOP_SERVICE_NAME,
        service_type=ClusterSlalomTfsStop,
        service_request=create_slalom_clustering_stop_srv_request(
            output_child_frame_ids=(
                [
            ...
            output_parent_frame_id=WORLD_FRAME,
    ...
    seq_yaw_and_recluster.add_children(
        [
            ...
            cluster_srv_start_yaw,
            py_trees.timers.Timer(duration=RECLUSTER_DURATION),
            cluster_srv_stop_yaw,
    ...
    cluster_srv_start_sweep_pre = py_trees_ros.service_clients.FromConstant(
        name=f"Recluster Transforms from Layer {current_layer} to Layer {next_layer} start (sweep)",
        service_type=ClusterSlalomTfsStart,
        service_name=CLUSTERING_START_SERVICE_NAME,
        service_request=create_slalom_clustering_start_srv_request(
            input_child_frame_ids=(
    ...
    cluster_srv_stop_sweep_pre = py_trees_ros.service_clients.FromConstant(
        name=f"Recluster Transforms from Layer {current_layer} to Layer {next_layer} stop (sweep)",
        service_name=CLUSTERING_STOP_SERVICE_NAME,
        service_type=ClusterSlalomTfsStop,
        service_request=create_slalom_clustering_stop_srv_request(
            output_child_frame_ids=(
    ...
    cluster_srv_start_sweep_1 = py_trees_ros.service_clients.FromConstant(
        name=f"Recluster Transforms from Layer {current_layer} to Layer {next_layer} start (sweep)",
        service_type=ClusterSlalomTfsStart,
        service_name=CLUSTERING_START_SERVICE_NAME,
        service_request=create_slalom_clustering_start_srv_request(
    ...
    cluster_srv_stop_sweep_1 = py_trees_ros.service_clients.FromConstant(
        name=f"Recluster Transforms from Layer {current_layer} to Layer {next_layer} stop (sweep)",
        service_name=CLUSTERING_STOP_SERVICE_NAME,
        service_type=ClusterSlalomTfsStop,
        service_request=create_slalom_clustering_stop_srv_request(
    ...
    cluster_srv_start_sweep_2 = py_trees_ros.service_clients.FromConstant(
        name=f"Recluster Transforms from Layer {current_layer} to Layer {next_layer} start (sweep)",
        service_type=ClusterSlalomTfsStart,
        service_name=CLUSTERING_START_SERVICE_NAME,
        service_request=create_slalom_clustering_start_srv_request(
    ...
    cluster_srv_stop_sweep_2 = py_trees_ros.service_clients.FromConstant(
        name=f"Recluster Transforms from Layer {current_layer} to Layer {next_layer} stop (sweep)",
        service_name=CLUSTERING_STOP_SERVICE_NAME,
        service_type=ClusterSlalomTfsStop,
        service_request=create_slalom_clustering_stop_srv_request(
    ...
    seq_sweep_and_recluster.add_children(
        [
            ...
            cluster_srv_start_sweep_pre,
            py_trees.timers.Timer(duration=SWEEP_RECLUSTER_DURATION),
            cluster_srv_stop_sweep_pre,
            goto_sweep_0,  # Sweep left
            cluster_srv_start_sweep_1,
            py_trees.timers.Timer(duration=SWEEP_RECLUSTER_DURATION),
            cluster_srv_stop_sweep_1,
            # recluster_sweep_0,  # Recluster during the left sweep
            goto_sweep_1,  # Sweep right
            cluster_srv_start_sweep_2,
            py_trees.timers.Timer(duration=SWEEP_RECLUSTER_DURATION),
            cluster_srv_stop_sweep_2,
